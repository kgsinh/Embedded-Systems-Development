/*******************************************************************************
 * File Name          : main.c
 * Description        : It will turn ON the RED LED when ADC Values are from 0 to 1365, RED + GREEN
 *                      when ADC values are from 1366 to 2730 and BLUE when ADC values are from 2731 to 4095.
...
// FUNCTION      : RGB
// DESCRIPTION   : It will generate RGB colors as per our requirements.
// PARAMETERS    : parameters passed are B, G and R.
// RETURNS       : None.
void RGB(unsigned int B, unsigned int G, unsigned int R) {
	TIM1->CCR1 = B; //Blue
...
int main(void) {
	...
	while (1) {
		...
		if (uhADCxConvertedValue >= 0 && uhADCxConvertedValue <= 1365) {
			RGB(0, 0, 65535); // RED ON
			printf("RED\r\n"); //Print RED on the screen
			HAL_Delay(100);
		} else if (uhADCxConvertedValue > 1365 && uhADCxConvertedValue <= 2730) {
			RGB(0, 65535, 65535); // RED + GREEN ON
			printf("RED + GREEN\r\n");
			HAL_Delay(100);
		} else if (uhADCxConvertedValue > 2730 && uhADCxConvertedValue <= 4095) {
			RGB(65535, 0, 0); //BLUE ON
			printf("BLUE\r\n");
			HAL_Delay(100);
		} else {
			RGB(0, 0, 0); // RGB LED OFF
		}
	}
	/* USER CODE END 3 */
